...
final class RollbackCommand extends Command
{
    ...
    protected $signature = 'deploy:rollback
                            {id : The deployment ID to rollback to}
    ...
    public function handle(): int
    {
        ...
        try {
            $deployment = $client->rollbackDeployment($siteId, $envId, $deploymentId);

            if ($this->shouldOutputJson()) {
                $this->outputJson($deployment);
            } else {
                $this->outputSuccess("Rollback initiated to deployment {$deploymentId}");
                $this->output->writeln('');
