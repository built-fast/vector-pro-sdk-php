...
final class ResetSftpPasswordCommand extends Command
{
    use FormatsOutput;
     ...
     * @var string
     */
    protected $signature = 'site:reset-sftp-password
                            {id : The site ID}
                            {--json : Output as JSON}
     ...
     * @var string
     */
    protected $description = 'Reset the SFTP password for a site';
    ...
    public function handle(): int
    {
        ...
        try {
            $result = $client->resetSiteSftpPassword($siteId);

            if ($this->shouldOutputJson()) {
                $this->outputJson($result);
            } else {
                $password = $result['password'] ?? $result['sftp_password'] ?? null;
                if ($password !== null) {
                    $this->outputSuccess('SFTP password reset successfully.');
                    $this->output->writeln('');
                    $this->output->writeln("<info>New Password:</info> {$password}");
                } else {
                    $this->outputSuccess('SFTP password reset successfully. Check your email for the new password.');
                }
            }
