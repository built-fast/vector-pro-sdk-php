...
describe('DeploymentsApi', function () {
    it('lists deployments', function () {
        $http = createHttpClient([
            ...
            expect($request->getUri()->getPath())->toBe('/api/v1/vector/environments/env-456/deployments');
        });
    ...
    it('lists deployments with pagination', function () {
        ...
        $api = new DeploymentsApi($http);
        $result = $api->list('env-456', ['per_page' => 10, 'page' => 2]);
    ...
    it('creates a deployment', function () {
        ...
        $api = new DeploymentsApi($http);
        $result = $api->create('env-456', ['description' => 'Release v1.0']);
    ...
    it('gets a deployment', function () {
        $http = createHttpClient([
            ...
            expect($request->getMethod())->toBe('GET');
            expect($request->getUri()->getPath())->toBe('/api/v1/vector/deployments/deploy-789');
        });

        $api = new DeploymentsApi($http);
        $result = $api->get('deploy-789');
    ...
    it('rolls back to a deployment', function () {
        $http = createHttpClient([
            ...
            expect($request->getMethod())->toBe('POST');
            expect($request->getUri()->getPath())->toBe('/api/v1/vector/environments/env-456/rollback');
            $body = json_decode($request->getBody()->getContents(), true);
            expect($body['deployment_id'])->toBe('prev-deploy');
        });

        $api = new DeploymentsApi($http);
        $result = $api->rollback('env-456', ['deployment_id' => 'prev-deploy']);
